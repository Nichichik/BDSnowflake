...
Перед выполнением SQL-скриптов необходимо загрузить исходные данные.

1. Подключиться к базе данных через **DBeaver**
2. Создать таблицу **`mock_data`** (staging table)
3. Импортировать все CSV файлы из папки **`исходные данные`**
4. Импорт выполняется через **Tools → Import Data → CSV**

Все файлы `MOCK_DATA.csv – MOCK_DATA (9).csv` должны быть загружены в одну таблицу **`mock_data`**.
...
После загрузки исходных данных необходимо выполнить SQL-скрипт:
```bash
scripts/ddl.sql
```

Этот скрипт создаёт следующие таблицы:
...
После создания таблиц необходимо выполнить SQL-скрипт:
```bash
scripts/dml.sql
```

Скрипт заполняет таблицы измерений и таблицу фактов, трансформируя данные из таблицы `mock_data`.
